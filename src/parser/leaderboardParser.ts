import { GDLeaderboardUser } from '../types/gd.js';

export function parseLeaderboardResponse(rawString: string): GDLeaderboardUser[] {
    if (!rawString || rawString === '-1') return [];
    const userStrings = rawString.split('|');
    const users: GDLeaderboardUser[] = [];
    for (const userStr of userStrings) {
        if (!userStr) continue;
        const parts = userStr.split(':');
        const data: Record<string, string> = {};
        for (let i = 0; i < parts.length; i += 2) { data[parts[i]] = parts[i + 1]; }
        users.push({
